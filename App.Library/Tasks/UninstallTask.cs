...
namespace App.Library.Tasks
{
    public class UninstallTask
    {
        ...
        public static void Uninstall(bool showMessageBox)
        {
            ...
            if (SelfInstaller.DefaultInstance.EnsureIsUserUninstalled() && showMessageBox)
            {
                var messageBoxProcess = new ProcessStartInfo
                {
                    FileName = "mshta",
                    Arguments = $"vbscript:Execute(\"msgbox \"\"{Resources.UninstallNotification}\"\", 0, \"\"{string.Format(Resources.UninstallNotificationTitle, Settings.Settings.AppTitle)}\"\":close\")",
